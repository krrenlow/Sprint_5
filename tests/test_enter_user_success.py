...
from urls import URLs  # Добавляем импорт URLs
from selenium.common.exceptions import TimeoutException  # Добавляем импорт исключения
import pytest
...
class TestUserAuthentication:
    def test_successful_login(self, driver):
        ...
        1. Открытие формы входа
        2. Ввод учетных данных
        3. Проверка успешной авторизации
        """
        try:
            # 1. Открываем главную страницу
            driver.get(URLs.BASE_URL)  # Используем URLs.BASE_URL вместо Config.BASE_URL
            ...
            # Дополнительная проверка URL (если нужно)
